...
- [ ] Reste sur la page Musique (identifié mais pas encore corrigé) :
  - Catégories "Disco"/"Rock" bidons : `Track` n'a aucun champ genre, les deux boucles Twig itèrent sur la même liste `tracks` en entier (doublons garantis s'il y avait plus d'un morceau)
  - `.loggedinContent` (liens de téléchargement pour les membres connecté·es) a `d-none` codé en dur, jamais retiré : invisible même connecté·e
  - `Player.js` plante si la playlist est vide (`links[0]` undefined) : pas de garde, à corriger même si ça n'arrive pas avec les fixtures normales
  - Layout entassé dans un cadre fixe 642×~430px avec 3 scrollbars internes différentes (lecteur / accordéon playlist / panneau membre)
