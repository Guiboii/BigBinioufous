...
- Sécurité (`config/packages/security.yaml`) : `form_login`, provider Doctrine sur `User` (identifiant = email, `getUserIdentifier()`), `password_hashers` bcrypt (`encoders` renommé depuis 5.3). `/admin/*` réservé à `ROLE_ADMIN`, `/desk/*` réservé à `ROLE_USER`. `user_checker: App\Security\UserChecker` bloque réellement la connexion tant que `User::$validation` n'est pas passé à `true` par un·e admin (aucune inscription n'est auto-validée depuis la simplification de l'inscription, cf. ci-dessous).
